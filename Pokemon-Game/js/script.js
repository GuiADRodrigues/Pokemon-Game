...
function verifyLookPokemon(to) {
    ...
    if(findCharmander) {
        const newTopPosition = (to = "ArroUp" ? `${getTopPosition() + 8}px` : `${getTopPosition() - 8}px`);
        charmander.style.right = pokemonRightPosition;
        charmander.style.top = newTopPosition;
    }

    if(findPikachu) {
        const newTopPosition = (to = "ArroUp" ? `${getTopPosition() + 36}px` : `${getTopPosition() - 36}px`);
        pikachu.style.right = pokemonRightPosition;
        pikachu.style.top = newTopPosition;
    }

    if(findZubat) {
        const newTopPosition = (to = "ArroUp" ? `${getTopPosition() + 80}px` : `${getTopPosition() - 80}px`);
